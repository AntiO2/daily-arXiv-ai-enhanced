...
TL;DR: 本文提出AuthentiCity，一个跨五大城市、支持多源数据融合与溯源追踪的3D城市知识图谱，并引入两类基准测试（自然语言查询翻译和图表示学习），实验表明现有大模型在此任务上表现有限。


<details>
  <summary>Details</summary>
Motivation: 现有城市数字孪生数据集缺乏统一表示来整合多源数据（权威、众包、机器学习、重建数据），且无法同时支持溯源追踪、空间推理和机器学习。

Method: 构建基于标记属性图的3D城市知识图谱，整合CityGML与OpenStreetMap数据，加入屋顶材料预测和LoD3重建几何；采用溯源模型确保衍生信息不替代权威数据；通过置信度加权边解决跨源对应关系，构建规范城市实体并保留可追溯链接。

Result: 数据集包含180 GiB、1.8亿节点、2.2亿边、12亿属性和360万建筑。强商用LLM执行准确率仅54-69%，7B开源模型仅6-19%，且无法识别不可回答的问题（从不弃权）。

Conclusion: AuthentiCity是首个大规模、多源、溯源感知的3D城市知识图谱，揭示现有语言模型在复杂空间推理和溯源感知查询上的严重不足，为未来研究提供了数据与评估基准。
...
TL;DR: 对9种采样策略（盲采与代理引导）在数据质量画像任务上进行了系统基准测试，发现盲采（特别是随机均匀采样和聚类采样）在所有规模和数据集上一致优于代理引导方法，后者因IQR代理偏差导致精度差、速度慢。


<details>
  <summary>Details</summary>
Motivation: 数据质量画像对数据驱动AI流程至关重要，但全量扫描在近实时监控中速度过慢。渐进式采样是标准替代方案，但哪种策略能在大规模下最佳保持画像保真度是开放问题。

Method: 在3个真实数据集（~50万行）、1个IoT流（230万行）、2个超大规模真实数据（~740万行）及合成数据（~500万行）上，对9种采样策略（随机均匀、几何、Yamane、聚类、Metropolis-Hastings、DAG、分层、重要性加权）进行基准测试，比较精度、运行时间及扩展性。

Result: 随机均匀采样在5%预算下对NYC 311数据平均相对误差0.49%；DAG引导MCMC达19.5%（差约40倍）；所有真实数据上DAG比随机均匀差11-49倍（p=0.002）。随机均匀近线性扩展（O(N^0.964)），DAG超线性（O(N^1.272)），在大数据上慢28-47倍且精度差6倍。

Conclusion: 代表性而非领域知识决定采样器质量；无模式的随机均匀或聚类采样足以用于生产级质量画像。代理引导方法因过度追求数值型离群点而忽视分类列中的质量缺陷，导致根本性失败。
...
TL;DR: MemLens是一个面向LLM智能体的价值感知记忆管理系统，通过Shapley值评估、分层可视化和交互式仪表盘，实现高效、可解释、个性化的长期记忆管理。


<details>
  <summary>Details</summary>
Motivation: 现有LLM记忆系统采用粗粒度的效用无关方式管理异构交互记录，导致冗余和低价值记录持续存在，亟需一种价值感知的记忆管理方法。

Method: 将记忆记录作为一等数据对象，构建端到端交互分析仪表盘，包含Shapley-style记忆评估、价值感知存储和记忆辅助响应三个模块，支持记忆值检查、分层结构可视化和多策略对比。

Result: 通过study-copilot应用验证，在响应质量、检索延迟和token消耗等维度上展现出更优性能。

Conclusion: MemLens可作为LLM智能体的高效、可解释、个性化长期记忆管理系统，解决了粗粒度记忆管理中的冗余和低价值问题。
...
### [4] [Beyond "What to Retrieve": Uncertainty in Retrieval-Augmented Code Generation](https://arxiv.org/abs/2607.24884)
*Chandan Kumar Sah,Xiaoli Lian,Li Zhang*

Main category: cs.SE

TL;DR: OpenCoder是一个不确定性感知框架，通过估计证据来源的不确定性来过滤和排序异构证据，从而提升仓库级代码生成质量。在RepoExec-inline基准上，GPT选择输出的正确率从56.25%提升到78.13%，但效果依赖于LLM后端，且与验证-修复基线持平。


<details>
  <summary>Details</summary>
Motivation: 仓库级代码生成依赖异构证据（相似代码、仓库上下文、项目API等），但这些证据在相关性、兼容性和完整性上存在内在不确定性。现有检索增强方法只优化检索相关性，没有显式建模证据不确定性对下游生成的影响。

Method: 提出OpenCoder框架：1) 估计各证据来源的不确定性；2) 基于不确定性对异构证据进行过滤和排序；3) 将不确定性作为控制信号指导生成、验证和修复。通过因子分析研究API知识、仓库上下文和相似代码三类证据的交互效应。

Result: 在扩展的32任务RepoExec-inline评估上，GPT条件下选择输出正确率从56.25%提升至78.13%，但与验证-修复基线持平。Gemini上的改进无统计显著性。目标感知的API精炼显著提升了API集合检索。

Conclusion: 不确定性可以作为仓库级检索、验证和修复的可操作控制信号。但效果高度依赖LLM后端，且没有普适的证据来源排序，跨来源交互显著。
...
### [5] [Characterizing Structural Testability in JavaScript: An Empirical Study](https://arxiv.org/abs/2607.24965)
*Shahrzad Mirzaei,Saba Alimadadi*

Main category: cs.SE

TL;DR: 本文通过大规模实证研究，提出了JavaScript结构可测试性的七维框架（包括可控性、可观测性、分支复杂度等），并基于30个开源项目分析了其分布特征和结构原型。


<details>
  <summary>Details</summary>
Motivation: 现有可测试性研究主要面向面向对象和并发软件，对JavaScript的异步、事件驱动、闭包等独特特性缺乏针对性研究，亟需建立适用于现代JavaScript系统的结构可测试性分析框架。

Method: 基于AST静态分析，构建了包含可控性、可观测性、分支复杂度、异步协调、事件驱动行为、封装性和副作用强度七个维度的可测试性框架，聚合为复合可测试性得分（CTS），并对30个开源JavaScript项目进行大规模分析。

Result: 发现结构上难以测试的函数集中在少量文件中，且源于多种重复出现的结构配置模式，而非单一主导模式。

Conclusion: 该研究为JavaScript结构可测试性提供了新见解，为未来测试工作量评估、自动测试生成、可测试性感知重构及软件质量评估奠定了基础。
...
### [6] [Preliminary Guidelines for Using and Evaluating GenAI Tools to Support Systematic Literature Reviews](https://arxiv.org/abs/2607.24991)
*Barbara Kitchenham,Sebastián Pizard,Lech Madeyski,Ronnie de Souza Santos,Martin Shepperd,David Budgen*

Main category: cs.SE

TL;DR: 本文提出GUEST建议，指导软件工程研究人员在使用生成式AI（GenAI）辅助系统文献综述（SLR）时，确保其严谨性、可靠性和透明度，并强调GenAI需要人类监督，不能独立完成系统性研究。


<details>
  <summary>Details</summary>
Motivation: GenAI和LLM越来越多地被用于学术任务（包括系统文献综述），但它们能否满足SLR所需的严谨性、可靠性和透明度尚无保证。作者旨在为使用GenAI进行SLR的研究人员以及评估GenAI支持SLR效果的实证研究提供指导。

Method: 1) 进行快速综述，识别关于评估和使用GenAI/LLM支持SLR的现有指南；2) 结合思想实验、文献中的相关指导以及自身经验，制定在SLR中使用和评估GenAI的建议。

Result: 识别出评估GenAI用于SLR时面临的问题，解释在规划、执行和报告使用GenAI的SLR及评估GenAI工具时需考虑的过程问题，最终总结为GUEST（GenAI Use and Evaluation in SLR Tasks）建议集。

Conclusion: GenAI需要人类监督，目前无法独立完成系统性研究，但可在重复性任务和复杂任务验证中提供经济有效的辅助。GUEST建议有助于软件工程研究人员进行可信的SLR和严格的评估研究。
...
### [7] [Authoring Agent Skills: A Software-Engineering Approach](https://arxiv.org/abs/2607.25032)
*Giuseppe Destefanis*

Main category: cs.SE

TL;DR: 本文论证Agent Skill应作为软件制品遵循软件工程原则（单一职责、接口与实现分离、低耦合、共享token预算经济性、行为评估替代确定性测试），并以Claude Code为参考实现，解析技能结构、分阶段加载机制、描述编写要点，将其与项目记忆文件、斜杠命令、子智能体、外部工具连接、钩子等行为塑造机制进行对比，提出基于"谁决定运行机制"和"提供什么保证"的选择规则，最后给出评估驱动的编写流程、常见模式与缺陷及第三方信任问题。


<details>
  <summary>Details</summary>
Motivation: 当前Agent Skill作为一种新兴的LLM智能体扩展方式，缺乏系统化的构建方法论。作者希望将软件工程原则引入技能构建，规范技能结构、加载机制和选择标准，并与现有行为塑造机制进行清晰对比，为开发者提供可操作的指导规则。

Method: 以Claude Code为参考实现，描述了技能的结构化组织方式、分阶段加载（description→full content）的机制、描述编写对技能选择的关键作用；将技能与项目记忆文件、斜杠命令、子智能体、外部工具连接、钩子五种机制进行系统比较，提出基于"谁决定运行"和"提供什么保证"两个维度的选择规则；最后给出评估驱动的编写流程、常见模式/缺陷及第三方信任分析。

Result: 提出了技能作为软件制品应遵循的原则体系（SRP、接口/实现分离、低耦合、token经济性、行为评估）；建立了六种行为塑造机制的对比框架和选择规则；识别了技能编写的常见模式与缺陷；揭示了第三方技能的信任问题。

Conclusion: Agent Skill的构建应系统化采用软件工程原则，通过行为评估而非确定性测试来保证质量；开发者应根据谁决定机制运行（用户/模型/代码）以及机制提供什么保证（指导/约束/强制）来合理选择行为塑造方式；第三方技能引入带来了信任挑战。
...
### [8] [What Motivates Whom? A Survey of Newcomers to OSS and Experienced OSS Practitioners](https://arxiv.org/abs/2607.25126)
*Shashiwadana Nirmani,Hourieh Khalajzadeh,Mojtaba Shahin,Xiao Liu*

Main category: cs.SE

TL;DR: 本研究通过对208名从业者的在线调查，探究OSS项目贡献者的项目选择偏好及其与动机、人口统计学背景的关系，发现年龄、性别、OSS角色等人口因素与动机显著相关，项目特征偏好随动机变化，并比较了新人与有经验者的差异，最后探讨了推荐系统的改进方向。


<details>
  <summary>Details</summary>
Motivation: 软件从业者在选择适合的OSS项目时面临困难，导致低效的入职和参与度下降。理解贡献者如何选择OSS项目对于支持他们的入职、参与和长期贡献至关重要。

Method: 采用在线问卷调查方法，收集了208名从业者的数据，分析其项目选择偏好、动机与人口统计学背景之间的相关性，并对比了新人与有经验者的不同偏好。

Result: 年龄、性别和OSS角色等人口因素与贡献者动机显著相关；项目年龄、开发阶段、文档质量等特征偏好因具体动机而异；新人与有经验从业者的项目选择偏好存在明显差异。

Conclusion: 研究揭示了新人与有经验者不同的项目选择需求，为改进OSS项目入职、参与和留存提供了洞见，同时为未来的项目推荐系统设计和OSS生态优化提供了指导。
...
### [9] [Learning from 53.6K Real-World Developer Edits of AI-Generated Code](https://arxiv.org/abs/2607.25130)
*Jenny T. Liang,Mihika Bairathi,Wayne Chi,Ameet Talwalkar,Nishant Subramani,Valerie Chen*

Main category: cs.SE

TL;DR: DECODE是一个包含53.6K真实IDE内AI生成代码编辑行为的数据集，来自1000+开发者，覆盖Python/TypeScript/JavaScript。研究发现大多数编辑发生在接受AI补全后15分钟内，31%的编辑轨迹最终删除了AI补全内容。基于DECODE微调的3B开源模型在代码编辑预测任务上显著优于前沿大模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM训练主要依赖Git提交数据，但Git只记录最终成功的代码片段，缺乏中间编辑过程的细粒度信息。高质量、真实的代码编辑数据缺失，限制了AI编程助手的改进。

Method: 收集了1000+开发者在IDE中编辑AI生成代码的真实行为数据（53.6K编辑），覆盖Python/TypeScript/JavaScript。用于数据分析洞察编辑行为和基准测试LLM的代码编辑预测能力。

Result: 1) 大多数编辑发生在接受AI补全后15分钟内；2) 31%的编辑轨迹最终删除了AI补全内容；3) 基于DECODE微调的3B开源模型在代码编辑预测任务上表现优于前沿LLM。

Conclusion: 强调了以开发者为中心的机器学习方法对未来AI编程助手的重要性，DECODE数据集为训练更贴近实际开发行为的代码编辑模型提供了宝贵资源。
...
### [10] [Specification-Driven DevOps for Multi-Service Environments](https://arxiv.org/abs/2607.25141)
*Oleg Grynets,Kyrylo Fursov,Vasyl Lyashkevych,Volodymyr Veres*

Main category: cs.SE

TL;DR: 本文研究前沿LLM能否仅凭仓库内容生成可执行的多服务Docker部署配置。实验发现：LLM能生成功能可运行的Dockerfile，正确重建服务拓扑、端口、代理等结构，但系统性地遗漏了网络隔离、多阶段构建、缓存优化、生产级前端服务等关键部署意图，暴露出功能正确性与部署意图保真度之间的根本性差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM生成的部署环境虽能功能执行，但未必符合架构安全、工作流和生产环境的真实意图。研究者希望量化评估LLM在多服务应用部署配置上的能力边界，并区分"功能正确"与"部署意图一致"这两个不同概念。

Method: 选取三个异构仓库（混合Python、Node.js、.NET、React、Rust、Java、Redis、PostgreSQL、MySQL等），由LLM生成Dockerfile和Docker Compose配置。使用确定性端到端HTTP测试作为功能验证依据，并结合人工结构对比分析生成结果。

Result: 所有三个生成环境功能上均可运行（仅一个需要将Rust基础镜像从1.85更新到1.88）。模型正确重建了服务拓扑、端口、基础设施依赖、服务主机名、后台worker、隐藏代理配置及基于文件的Docker secrets机制。但系统性遗漏了网络分段、多阶段构建、依赖层缓存、热重载卷、生产前端服务、后端端口限制策略及跨平台构建逻辑。

Conclusion: 研究正式界定了功能正确性与部署意图保真度的区别，并从分析推导出：某些部署信息无法从仓库制品中可靠推断，需要最小显式部署规范来弥补LLM在部署配置生成上的固有局限。
...
### [11] [NFR-to-Code Traceability in a Blockchain-IoT System: An Empirical Study](https://arxiv.org/abs/2607.25325)
*Yifei Wang,Jacky Keung,Xiaoxue Ma,Shijie Zhang,Yishu Li*

Main category: cs.SE

TL;DR: 本文研究了非功能需求（NFR）到代码的可追溯性问题，基于真实的区块链-IoT项目构建了包含FR和NFR链接的标注数据集，评估了四种检索基线方法。结果表明FR追溯比NFR更容易，安全相关NFR最难，主要挑战在于NFR的实现证据分散且不局限于代码局部。


<details>
  <summary>Details</summary>
Motivation: 现有需求到代码的追溯研究主要聚焦于功能性需求（FR），而对非功能性需求（NFR）的追溯研究不足，这阻碍了安全、可靠性等关键质量属性的验证。

Method: 基于真实区块链-IoT项目，设计标注协议构建跨异构工件的追溯链接，创建包含FR和NFR链接的人工标注子集，并评估四种检索基线方法：TF-IDF、BM25、LSI和WMD。

Result: FR到代码的追溯始终比NFR更容易；安全相关的NFR是最难追溯的子类；NFR追溯的主要挑战不在于需求的可用性，而在于实现证据在代码中分布分散且不明确局部化。

Conclusion: NFR到代码的可追溯性比FR更困难，尤其是安全需求；核心挑战是代码中NFR实现证据的分散性，需要针对性的方法来解决这一问题。
...
### [12] [Specula: Scaling formal specifications for autonomous model checking of system code](https://arxiv.org/abs/2607.25333)
*Qian Cheng,Saad Mohammad Rafid Pial,Ruize Tang,Yiming Su,Emilie Ma,Finn Hackett,Ivan Beschastnikh,Yu Huang,Tianyin Xu*

Main category: cs.SE

TL;DR: Specula是一个自动化系统，利用基于LLM的智能体为大型系统代码生成高质量TLA+形式化规范，并通过自进化循环持续改进规范质量，成功在48个开源项目中发现249个bug。


<details>
  <summary>Details</summary>
Motivation: 消除将形式化方法应用于真实世界系统代码的障碍（传统方法依赖人类专家），同时解决LLM驱动技术中的奖励黑客和幻觉问题。

Method: 使用基于LLM的编码智能体自主开发TLA+规范（包括不变量和形式化模型），通过自进化循环迭代提升规范质量，加深智能体对系统代码及其行为的理解。

Result: 在48个开源系统项目中发现了249个bug，包括许多现有方法难以发现的深层bug，已被多家公司使用。

Conclusion: Specula实现了全自主的形式化验证，有效降低了形式化方法的应用门槛，并能高效发现真实系统中的深层缺陷。
...
### [13] [CodeNib: A Multi-View Data System for Serving Repository Context to Coding Agents](https://arxiv.org/abs/2607.25431)
...
TL;DR: CodeNib构建了一个统一运行时，为代码智能体提供可重用的词法、密集和结构视图，并支持跨提交的搜索、符号导航和上下文服务。实验表明，在质量保持的前提下，图更新和向量更新速度分别提升8.7倍和25.4倍，上下文令牌减少50-87%。


<details>
  <summary>Details</summary>
Motivation: 现有编码智能体在浏览仓库时，需要反复进行索引构建、语言服务器查询和任务历史记录维护，导致大量重复发现和生命周期成本被掩盖。缺乏统一的仓库上下文服务机制。

Method: CodeNib为每个仓库提交构建可重用的词法、密集和结构视图；将输出映射到仓库相对源范围；跨编辑维护选定视图；通过单一运行时提供排序搜索、符号导航和有限上下文服务。

Result: 在100个快照上，图更新和向量更新速度分别比独立重建快8.7倍和25.4倍（中位数）。在静态导航子集上，每请求的实时/静态延迟比为4.7倍。五种模型下，上下文策略相比配对grep/read减少50-87%的轨迹令牌。

Conclusion: CodeNib支持多视图仓库上下文服务，具有明确、操作特定的有效性边界，有效提升代码智能体的效率并降低冗余成本。
...
### [14] [Repositories, Contributors, and Continuity: An Empirical Study of Foundational Quantum Software](https://arxiv.org/abs/2607.25437)
*Vincent Gierisch,Nicole Hoess,Ralf Ramsauer,Wolfgang Mauerer*

Main category: cs.SE

TL;DR: 本文通过实证软件工程技术分析量子软件生态系统中的仓库演化，发现了贡献者迁移模式和跨项目知识转移的证据，揭示了概念和影响力如何跨越仓库边界持续存在。


<details>
  <summary>Details</summary>
Motivation: 量子软件生态系统快速成长，但许多仓库的开发活跃期有限，其概念是否能在单个仓库之外持续存在尚不清楚，因此需要研究跨仓库的概念延续性。

Method: 运用实证软件工程技术，对一组基础量子软件仓库进行分析，通过结合跨仓库活动和贡献者关系来研究社区的演化。

Result: 提供了贡献者迁移模式和跨项目知识转移的实证证据；观察到多种发展路径：项目演变为持续社区、贡献者将概念整合到已有生态系统中、或通过新的后续软件制品延续活跃度。
...
### [15] [Input Relation Prompting for Metamorphic Testing on Query-Based Systems](https://arxiv.org/abs/2607.25603)
*Eng-Shen Tu,Shin-Jie Lee*

Main category: cs.SE

TL;DR: 本文提出一种辅助测试人员识别蜕变关系（MRs）的方法，用于对查询型系统（QBSs）进行蜕变测试（MT），无需依赖预定义测试用例或真实基准，并通过实际Web应用案例验证了其可行性和有效性。


<details>
  <summary>Details</summary>
Motivation: 查询型系统测试面临两大挑战：缺乏用于验证的真实基准（ground truth），以及手动测试耗时耗力。现有方法过度依赖领域知识，亟需一种能辅助测试人员自动或半自动发现蜕变关系的方法，以缓解预言机问题。

Method: 提出一种辅助测试人员识别MRs的方法，通过提示输入与输出之间的变换规则（MRs），当预期关系未被满足时即可检测故障。该方法不依赖预定义测试用例或真实基准，且可与组合测试、模糊测试等其他方法结合使用。

Result: 通过一个真实Web应用的案例研究，展示了所提方法的可行性和潜力，证明其能有效辅助测试人员识别MRs并提升测试效率。

Conclusion: 该研究推进了蜕变测试领域的发展，为查询型系统测试人员提供了一种实用工具，可提升测试效率，且具有与其他测试方法结合的良好扩展性。
...
### [16] [SkillGate: Cost Efficient Runtime Malicious Skill File Detection in Coding Agents](https://arxiv.org/abs/2607.25619)
*Rui Yang,Michael Fu,Kla Tantithamthavorn,Chetan Arora,Joey Chua*

Main category: cs.SE

TL;DR: 本文提出SkillGate，一种针对AI编码代理技能文件供应链攻击的安全网关，通过混合正则预过滤+LLM判断流水线实现高效检测，在SkillsBench基准上F1=0.817，误报率1.13%，LLM输入token减少77%。


<details>
  <summary>Details</summary>
Motivation: AI编码代理技能文件可被恶意利用进行凭证窃取、后门注入等供应链攻击，现有公共注册表缺乏安全审查，且尚无系统性防御工具链。

Method: SkillGate采用混合正则预过滤+LLM判断流水线：安全信号文件直接放行（跳过LLM），可疑文件仅将匹配的片段窗口发送给LLM判断（而非完整内容），从而降低计算成本。

Result: 在SkillsBench（n=1,650，9.1%恶意）上，SkillGate达到F1=0.817，误报率仅1.13%，LLM输入token减少77%（相比全文件筛查），AUPRC达0.830，优于现有工具5-6倍（0.144/0.162）。

Conclusion: SkillGate有效填补了AI编码代理技能文件供应链攻击的防御空白，以较低开销实现了高检测率和低误报率，有望成为该领域的基础安全工具。
...
### [17] [An Empirical Study of Model Context Protocol Applications](https://arxiv.org/abs/2607.25635)
*Muhammad Hamza Arshad Majeed,May Mahmoud,Sarah Nadi*

Main category: cs.SE

TL;DR: 本文对GitHub上1,723个MCP应用进行大规模研究，发现生态在配置文件和SDK使用上趋于一致，但命名规范和人工监督机制仍存在显著差异，尤其是仅37.2%的应用设置了工具执行的审批步骤。


<details>
  <summary>Details</summary>
Motivation: MCP协议标准化了LLM应用与外部工具的通信，但应用端的配置、通信和人工监督缺乏约定，且现有研究集中于服务器端，对消费服务器的应用端研究不足。

Method: 从GitHub挖掘1,723个MCPApp，先通过代表性样本归纳出MCPAppTax分类体系，再借助LLM辅助管道对全部数据集进行标注和分析。

Result: 85.2%的应用使用配置文件集成服务器，81.1%使用官方SDK通信，但配置文件无统一命名规范；90.8%记录日志，77.2%支持启用/禁用控制，仅37.2%设置了审批步骤。

Conclusion: MCP应用生态在基础集成实践上已趋于收敛，但在配置标准化和人工监督机制上仍存在明显空白，尤其多数应用缺乏对工具调用的有效控制，存在安全风险。
...
### [18] [KQFuzz: Knowledge-Guided Fuzzing for Quantum Libraries via Large Language Models](https://arxiv.org/abs/2607.25647)
*Fuyuan Xia,Qixin Zhang,Chenhao Ying,Haojin Zhu,Shuai Wang,Yuan Luo,Pingchuan Ma,Yuxuan Du*

Main category: cs.SE

TL;DR: 本文提出KQFuzz，一种基于知识引导的量子库模糊测试工具，通过融合代码库知识、适应度评估和双层变异策略，显著提升测试覆盖率和bug发现效率，在三大量子库中发现13个bug。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的量子库模糊测试方法存在灵活性不足和效率低下的问题，制约了量子计算领域的进展，亟需更高效可靠的测试方法。

Method: 1) 针对量子程序的提示方案，融入代码库知识引导LLM生成高质量种子程序；2) 适应度引导的评估机制；3) 双层变异策略，探索复杂执行路径并触发潜在bug。

Result: 在Qiskit、PennyLane和Cirq三个量子库上，覆盖率最高提升18.44%，发现13个bug（12个已修复），显著优于现有方法。

Conclusion: 知识引导策略能有效提升量子库模糊测试的效率和效果，为量子计算软件的可靠性保障提供了有效途径。
...
### [19] [Delta Debugging for Cyber-Physical Systems with Flaky Test Executions](https://arxiv.org/abs/2607.25695)
*Pablo Valle,Shaukat Ali,Aitor Arrieta*

Main category: cs.SE

TL;DR: 本文提出三种适用于随机CPS环境的delta调试算法，通过统计分析、重复执行和环境感知缩减来定位最小故障触发输入，实验表明该方法不仅减少了调试时间，还能提高故障复现率。


<details>
  <summary>Details</summary>
Motivation: 现代CPS模拟器存在非确定性（flaky）行为，导致故障难以复现和调试。现有delta调试方法基于确定性假设，不适用于随机环境。

Method: 提出三种结合统计失败分析、重复执行和环境感知缩减的delta调试算法，用于在随机CPS中隔离最小故障触发输入。

Result: 在工业电梯调度系统和自主移动机器人两个案例中，该方法显著减少了调试时间，同时保留了原始故障行为。更重要的是，最小化故障触发输入反而提高了故障复现率。

Conclusion: Delta调试不仅能简化故障分析，还能通过消除引入偶然随机效应的执行片段来缓解执行flakiness，为CPS调试提供了实用基础。
...
### [20] [WarmTuner: Program-Specific Warm Starts for Compiler Autotuning via Offline-to-Online Reinforcement Learning](https://arxiv.org/abs/2607.25831)
*Tianlu Qiao,Mingxuan Zhu,Zeyu Sun,Dan Hao*

Main category: cs.SE

TL;DR: 提出WarmTuner，一种离线到在线的强化学习框架，通过学习程序条件策略来优化编译器标志配置，在30个程序中达到平均1.732x加速比。


<details>
  <summary>Details</summary>
Motivation: 现有编译器自动调优技术利用历史数据提取的知识在搜索开始后冻结，无法根据目标程序的运行时反馈调整先验知识，导致先验与目标程序不匹配时浪费大量预算。

Method: 提出WarmTuner框架：离线阶段从历史好配置学习程序条件策略（覆盖全标志空间）；在线阶段使用GRPO（组相对策略优化）在目标程序上利用真实编译-运行反馈精炼策略，避免额外价值模型。

Result: 在GCC 15.2.0上对cBench和PolyBench评估，平均加速比1.732x（对比-O3），在14/30个程序上取得最优结果，显著优于现有技术。

Conclusion: WarmTuner通过离线学习程序条件策略并在线适应目标程序，有效解决了编译器调优中历史先验与目标程序不匹配的问题。
...
### [21] [Rethinking Training Data for Generating Code Review Comments](https://arxiv.org/abs/2607.25851)
*Leonardo Centellas-Claros,Estefania Pakarati-Cofre,Juan Pablo Sandoval Alcocer,Diego Elias Costa*

Main category: cs.SE

TL;DR: 本文通过实证分析发现代码评审评论生成任务中存在三类“错位”训练实例（语义模糊、缺乏可操作性、上下文依赖），指出仅靠数据集清洗无法根本解决问题，需改进任务设计、上下文输入和评估标准。


<details>
  <summary>Details</summary>
Motivation: 现有自动化代码评审评论生成方法常产生泛化、弱依从或不可操作的评论，且数据集包含噪声/不合适的训练实例，但已有研究未深入分析这些问题的异质性及其与任务定义的深层关联。

Method: 对广泛使用的代码评审评论数据集进行经验性审查，识别diff-comment对中“错位”的训练实例，归纳出错位现象的三种来源分类体系（语义模糊、缺乏可操作性、上下文依赖），并探究将该分类体系融入LLM过滤的效果。

Result: 发现错位实例确实存在且可分类，但基于分类体系的LLM过滤在识别错位训练对方面仍具挑战性，说明问题不仅限于数据集质量。

Conclusion: 改进代码评审评论生成不能仅依赖数据集清洗，还需要建立明确的有效性标准、更丰富的上下文输入，以及与评审意图和可操作性对齐的评估实践。
...
### [22] [How Do LLMs Read Bug Reports? An Empirical Study of Attention in LLMs for Automated Program Repair](https://arxiv.org/abs/2607.25873)
*Ramtin Ehsani,Irene Manotas,Saurabh Pujar,Luca Buratti,Preetha Chatterjee*

Main category: cs.SE

TL;DR: 本文首次实证研究LLM在程序修复中的注意力模式，发现成功修复时模型注意力分散在多个诊断组件上，而失败时注意力过度集中在元数据上；模型注意力与开发者关注点的一致性越高，修复成功率也越高。


<details>
  <summary>Details</summary>
Motivation: LLM自动程序修复系统性能不一致，即使相同上下文下同一个模型也可能修复一个bug却失败于另一个相似bug，但这种现象的原因尚不明确，模型如何处理和优先关注bug报告中的信息也缺乏理解。

Method: 对SWE-bench Verified和Multi-SWE-bench中的319个真实Python和Java bug进行实证分析，通过三个研究问题（RQ1：注意力在bug报告各部分的分布；RQ2：成功与失败修复的注意力模式差异；RQ3：注意力模式与开发者认为重要信息的对比）来系统研究。

Result: 成功修复的特征是注意力分散在bug描述、堆栈跟踪和测试用例等多个诊断组件上；失败修复则表现出注意力过度集中在版本信息等元数据上；模型注意力与开发者识别的关键部分和短语的一致性越高，修复成功率越高。

Conclusion: 注意力分配不当是LLM程序修复失败的关键因素，这一发现为设计更可解释、更可靠的自动程序修复系统提供了可操作的见解。
...
### [23] [CONQuER: Hardware-Aware Mixed-Precision Quantisation with Online-Calibrated Surrogates](https://arxiv.org/abs/2607.25884)
*Aidan Dakhama,Ajitha Rajan*

Main category: cs.SE

TL;DR: CONQuER是一个统一编译器集成的硬件感知混合精度量化框架，通过将量化移入编译器流水线、结合NSGA-II进化算法与双代理预筛选引擎，在资源受限硬件上实现高达12.19倍的推理加速，且top-1精度仅下降1.44%。


<details>
  <summary>Details</summary>
Motivation: 当前混合精度量化流程高度碎片化：量化在硬件无关的前端框架中预处理，与后端编译器分离，导致分配的位宽与目标硬件（如张量核心、可变宽度向量单元）不匹配，造成严重运行时性能惩罚。同时，硬件在环测试因指数级搜索空间而难以处理。

Method: 1) 将量化移入编译器流水线（TOSA层级），实现编译器感知的配置处理；2) 采用NSGA-II进化算法耦合双代理预筛选引擎（评估理论缓存边界和特征空间各向同性）剔除不可行配置；3) 最强候选策略通过IREE在硬件上执行，运行指标反馈给在线校准器以对齐代理模型与真实硬件行为。

Result: 在移动CPU、笔记本CPU和服务器GPU上验证，最优量化策略具有硬件依赖性。CONQuER发现的Pareto最优配置可实现最高12.19倍的推理加速，同时top-1精度控制在未量化基线的1.44%以内。

Conclusion: 量化与编译器下挖及物理执行耦合是发现硬件最优量化策略的关键，CONQuER的编译器集成方法有效解决了传统流程的性能碎片化问题。
...
### [24] [RSIBench-Data: Benchmarking Data-Centric Research for Recursive Self-Improvement](https://arxiv.org/abs/2607.25886)
*Fanqing Meng,Lingxiao Du,Qiguang Chen,Ziqi Zhao,Haocheng Lu,Mengkang Hu,Michael Qizhe Shieh*

Main category: cs.SE

TL;DR: 本文提出了RSIBench-Data基准，用于评估LLM智能体作为数据驱动研究者的能力。实验表明智能体能进行基础数据研究（58.33%设置能改进初始策略），但改进不稳定，78.26%的搜索最终得分低于峰值，说明当前智能体无法将反馈转化为持续改进。


<details>
  <summary>Details</summary>
Motivation: 递归自我改进需要将模型失败证据转化为更好的模型，但现有基准将研究决策与优化、服务、评估等工程任务纠缠在一起，难以衡量智能体的真正研究能力。作者希望构建一个受控基准，专门评估LLM智能体作为数据驱动研究者的核心能力。

Method: 作者设计了RSIBench-Data基准，固定后训练堆栈，让智能体在固定目标模型上迭代修订训练数据策略；使用Tinker支持训练和服务，Harbor和E2B沙箱进行官方评估；在软件工程、终端使用、科学问答和数学六个基准上评估四个前沿智能体。

Result: 智能体在58.33%的设置中能通过反馈改进初始策略；但78.26%的搜索最终尝试得分低于历史最佳，其余仅恢复峰值。更强运行中识别出四种模式：准确假设、验证监督、行为对齐数据、保留强检查点。

Conclusion: 当前智能体能够做出有用的数据驱动发现，但无法将反馈转化为一致持续的改进。RSIBench-Data为递归自我改进所需的研究能力提供了可测量、可审计的测试平台。
...
### [25] [A Low-Cost Human-in-the-Loop Investigation of Toxicity on GitHub at Scale](https://arxiv.org/abs/2607.25946)
*Rahat Rizvi Rahman,Mia Mohammad Imran,Kostadin Damevski*

Main category: cs.SE

TL;DR: 本文提出了一种人机协同（HITL）的标注方法，利用小型本地LLM和随机森林验证器，实现了对GitHub讨论中隐性毒性内容的大规模、领域校准标注，并基于124,000+对话数据验证和修正了先前小规模研究的关键发现。


<details>
  <summary>Details</summary>
Motivation: 开源讨论中的毒性互动会疏远贡献者并威胁项目可持续性，但先前关于GitHub毒性的实证研究规模有限，且由于毒性内容隐晦且依赖上下文，大规模标注难以可靠实现。

Method: 提出人机协同（HITL）标注流程：先由小型本地LLM生成毒性预测和可解释的事件类别分数，再由轻量级随机森林验证器基于这些分数标记最可能误标的少数对话，仅对这些子集进行人工审查。

Result: 随机森林验证器在低标注成本下优于基于置信度和多LLM的基线方法；成功应用于124,000+个GitHub讨论，验证并修正了先前小规模研究的发现，并提供了新的见解。

Conclusion: 该HITL方法使大规模、领域校准的毒性标注成为可能，确认了部分先前研究发现，也修正了另一些结论，并揭示了跨开源项目的毒性普遍性、特征与动态变化。
...
### [26] [RepoReasoner: Evaluating Repository-Level Code Reasoning Ability of Long-Context Language Models](https://arxiv.org/abs/2607.25996)
*Yanlin Wang,Suiquan Wang,Yanli Wang,Bowen Zhang,Daya Guo,Jiachi Chen,Zibin Zheng*

Main category: cs.SE

TL;DR: 提出了RepoReasoner基准，用于评估大语言模型在仓库级别的代码推理能力，包含输出预测和调用链预测两个任务。实验发现最优模型Pass@1仅69.1%，模型存在依赖记忆而非真正推理的问题，且长上下文不一定带来提升。


<details>
  <summary>Details</summary>
Motivation: 现有代码推理基准局限于函数级别，所有相关信息都在局部范围内，无法反映真实软件开发中跨多文件和复杂依赖结构的推理需求。

Method: 构建多阶段pipeline：利用pytest动态追踪获取真实调用链，结合LLM-based I/O改写减少记忆效应。设计两个互补任务——输出预测（细粒度跨文件执行推理）和调用链预测（高维度架构依赖理解）。

Result: 最优模型在输出预测上Pass@1仅69.1%；调用链预测上精确率高但召回率低；改写数据上性能下降表明存在记忆依赖；长上下文因噪声反而未必提升效果。

Conclusion: 当前LLM在仓库级代码推理上存在根本性局限，需要推进结构化架构理解和跨文件推理方面的研究。
...
### [27] [Enhancing Error Detection Performance through Parallel CRC Computation on Multi-Core Architectures](https://arxiv.org/abs/2607.24849)
*Mohammad Javad Khani,Mahmood Ahmadi*

Main category: cs.DC

TL;DR: 本文提出了一种基于POSIX线程的多核架构通用并行CRC框架，支持多种CRC变体，采用GF(2)上的多项式算术和矩阵移位操作保证并行与串行计算等价性，实验表明在大数据集上可获得约3-4倍加速比。


<details>
  <summary>Details</summary>
Motivation: 传统软件CRC实现存在固有的顺序依赖，无法有效利用现代多核处理器的并行计算能力，亟需一种通用且正确性保持的并行CRC加速方案。

Method: 提出基于POSIX线程的通用并行CRC框架，支持CRC-8/16/32/64/128等多种变体；采用GF(2)上的多项式算术和基于矩阵的移位操作实现CRC组合，而非简单的XOR聚合，确保并行与串行计算结果的等价性。

Result: 在多个工作负载大小和线程配置下进行评估，包括执行时间、吞吐量、延迟、可扩展性和能耗分析。结果表明对于大数据集可达到约3-4倍加速比，同时保持CRC计算精确正确。

Conclusion: 该框架提供了一种可移植、通用的软件方案，能在通用多核系统上实现保持正确性的并行CRC加速，并与查表法、slicing-by-8、SIMD向量化CRC及硬件辅助CRC等方法进行了对比讨论。
...
### [28] [Spanergy: Energy-aware Distributed Tracing for Microservices](https://arxiv.org/abs/2607.24902)
*César Perdigão Batista,Denis Conan,Sophie Chabridon*

Main category: cs.DC

TL;DR: Spanergy提出了一种能量感知的分布式追踪方法，通过将每个微服务的功耗测量与追踪关联，将能耗归因到单个请求段（trace spans），实现了对微服务应用的低开销能耗观测。


<details>
  <summary>Details</summary>
Motivation: 现有云数据中心能耗持续增长，微服务应用缺乏细粒度的请求级能耗可见性；分布式追踪虽广泛用于诊断延迟和故障，但无法暴露单个用户请求的能量成本，限制了能量感知的调试、问责和控制。

Method: 提出Spanergy方法，将每个微服务的功耗测量与分布式追踪关联，将测得的能耗归因到请求的各个追踪段（trace spans）；支持同步请求链和微服务间的异步交互；基于OpenTelemetry实现。

Result: 启用OpenTelemetry追踪使总实验能耗增加59.1%，Spanergy后处理额外增加15.2%基准能耗；发现请求中不可忽略的部分能耗来自延迟关键路径之外的span。

Conclusion: 能量感知追踪以适度开销即可实现，并为能效微服务提供可操作的洞察；Spanergy的增量能耗成本小于启用追踪本身的能耗开销，表明该方法在实际中轻量级且可行。
...
### [29] [QCOEM: Quantum Cloud Orchestration with Evolutionary Multi-Objective Optimization](https://arxiv.org/abs/2607.25358)
*Tam N. Pham,Hoa T. Nguyen,Quan Le-Trung*

Main category: cs.DC

TL;DR: 提出了QCOEM框架，利用进化算法（NSGA-II/NSGA-III）多目标优化量子云任务调度，联合最小化完成时间、错误率和负载不均衡，并通过AASF从Pareto前沿选择偏好调度方案，相比噪声无关启发式方法实现了零重调度和约30%的保真度提升。


<details>
  <summary>Details</summary>
Motivation: 现有量子云平台调度器采用噪声无关的启发式方法，忽略了不同量子后端特有的噪声分布、量子比特拓扑和队列状态，导致执行保真度下降、负载不均衡和频繁重调度。

Method: 提出QCOEM框架：1) 采用NSGA-II和NSGA-III对平均完成时间、执行错误率和负载不均衡进行多目标联合优化；2) 使用增强成就标量化函数(AASF)从非凸Pareto前沿中选择符合用户偏好的单一可调度方案。

Result: 在异构量子云环境中实现零任务重调度，平均保真度比噪声无关启发式方法提高约30%，且调度开销可控。
...
### [30] [CW-Ghost: Search-Free Granularity Selection for Helper-Thread Prefetching via Capacity Windows](https://arxiv.org/abs/2607.25363)
*Ya Zhang,Tong Lei,Yao Chen,Yonggang Che,Chuanfu Xu,Haozhong Qiu,Yusong Tan*

Main category: cs.DC

TL;DR: CW-Ghost提出了一种基于缓存容量预算的动态粒度控制方法，通过单次离线分析估算每迭代的缓存行填充量，结合容量窗口和块级同步来优化辅助线程预取，在Intel和AMD平台上分别实现1.54x和1.33x的加速比。


<details>
  <summary>Details</summary>
Motivation: 辅助线程预取的固定覆盖范围无法适应不同负载和处理器，而穷举配置评估成本高昂，缺乏一种高效且自适应的粒度选择机制。

Method: 1) 单次离线性能分析估算目标区域每迭代的平均需求缓存行填充量；2) 结合缓存容量预算推导容量窗口，决定每个预取块的迭代粒度；3) 采用块级同步限制辅助线程超前主线程的块数量。

Result: 在14个工作负载上，Intel平台几何平均加速比1.54x，AMD平台1.33x；相比Ghost Threading性能提升15.8%和10.8%；在两个平台上均达到候选集内经验最优性能的99%以上。

Conclusion: 缓存容量约束可以有效指导辅助线程预取的粒度选择，CW-Ghost实现了接近最优的自适应预取性能。
...
### [31] [WASP: A Configurable Framework for Portable Stateful Serverless Applications](https://arxiv.org/abs/2607.25493)
*Matteo Cenzato,Dario d'Abate,Arianna Dragoni,Giacomo Orsenigo,Luca Tosetti,Alessandro Margara*

Main category: cs.DC

TL;DR: WASP是一个面向边缘-云连续体的可配置有状态WASM无服务器框架，通过解耦组件（WASM运行时、数据存储、生命周期策略）实现异构环境下的灵活部署，实验证明其零开销且适配多场景。


<details>
  <summary>Details</summary>
Motivation: 现有WASM无服务器平台与特定执行引擎紧耦合且仅支持无状态工作负载，无法满足边缘异构环境中对有状态应用及多样化硬件/工作负载约束的需求。

Method: 提出WASP框架，采用严格解耦的可插拔组件架构（WASM运行时、数据存储可互换），并引入可配置的生命周期与缓存策略，无需修改应用代码即可适配异构环境。

Result: 实验表明WASP运行时开销可忽略不计，且通过切换运行时、数据存储和策略可展现出显著不同的内存与延迟特征，验证了其在边缘-云连续体中的适应性。

Conclusion: WASP成功为边缘-云异构环境提供了灵活且有状态的无服务器执行能力，打破了现有平台的紧耦合限制。
...
### [32] [PowerScale: Energy-Efficient Geo-Distributed Model Training with Federated Datacenter Power](https://arxiv.org/abs/2607.25650)
*Talha Mehboob,Zhe Xu,Michael Zink,David Irwin*

Main category: cs.DC

TL;DR: PowerScale提出了一种分层聚合系统，通过区域集群+同步-异步混合同步机制，在广域分布式AI训练中实现与单层聚合相当的收敛速度，同时能耗降低最高3.9倍。


<details>
  <summary>Details</summary>
Motivation: 大规模AI训练中，单层聚合架构在跨地域、电力受限的站点间存在根本性的能效问题：同步屏障导致快站点空闲等待、全量WAN通信能耗过高、固定同步频率在训练后期更新量变小时仍支付相同通信成本。

Method: PowerScale采用分层聚合架构：将站点按网络邻近性和电力可用性组织为区域集群，集群内使用同步模式频繁聚合，集群间通过异步模式将预聚合更新推送到全局聚合器；并引入自适应同步策略，根据训练进度动态调整集群同步频率。

Result: 在100站点规模的Flower仿真环境中，PowerScale相比单层基线方法达到相同或略优的收敛时间，同时能耗降低最高3.9倍。

Conclusion: 分层聚合架构（区域同步+全局异步）能够有效解决广域分布式训练中的能效瓶颈，在不牺牲训练速度的前提下大幅降低通信能耗，为电力受限场景下的跨数据中心AI训练提供了可行方案。
...
### [33] [Hermes: Low Tail-Latency Via Prefix Consensus](https://arxiv.org/abs/2607.25916)
*Alejandro Ranchal-Pedrosa,Dakai Kang,Neil Giridharan,Dahlia Malkhi,Mohammad Sadoghi,Ben Marsh*

Main category: cs.DC

TL;DR: Hermes是一种基于前缀共识的两轮循环领导者BFT协议，在n=5f+1的异步环境下，允许过期视图也能完成最终确认，解决了传统领导者协议中超时带来的延迟问题。


<details>
  <summary>Details</summary>
Motivation: 传统基于领导者的BFT协议中，领导者崩溃或缓慢会导致视图无法完成最终确认，超时设置面临保守（长时间停滞）和激进（浪费有效视图）的两难，这种权衡主导了尾延迟而非理想情况。

Method: Hermes使用前缀共识，投票携带按前缀关系排序的值，仲裁要求可比性而非等值。每个进程在视图开始时广播合理化提案并投一票（交付时投领导者提案，超时时投后备提案），无需空投票。通过n=5f+1的设置和父相对delta tipcuts及显式跳过机制保持独立提案的可比性。

Result: 及时诚实的领导者在2δ内完成完整提案的最终确认；否则任意n-f个投票（无需匹配）能最终确认最重的公共前缀。复杂度为O(n²m̂ + λn³)比特和O(n²)消息每视图。

Conclusion: Hermes消除了超时导致视图浪费的问题，在尾延迟性能上优于传统领导者BFT协议，并可作为最终确认组件集成到可用链和Autobahn多通道传播系统中。
